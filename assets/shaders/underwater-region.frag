...
void main() {
    ...
    float distortAmount = clamp(texture2D(u_distortmap, vec2(worldc.x / u_ww, worldc.y / u_wh)).r,0.0,1.0);
    ...
    gl_FragColor = vec4(sampled.r-0.2*distortAmount,sampled.g-0.2*distortAmount,sampled.b+0.05*distortAmount,sampled.a);
    ...
    if(distortAmount<0.05 && distortAmount > 0) {
        float dist = 1-abs(distortAmount-0.025)/0.025;
        float eff = dist*0.5;
        gl_FragColor = lerp(gl_FragColor,vec4(0.5,0.7,1.0,1.0),eff);
    }
    //heatmap();
}
