package subvoyage.type.unit.weapon;
...
public class DroneWeapon extends Weapon {

    public UnitType drone;

    public DroneWeapon(String name) {
        super(name);
        mountType = DroneMount::new;
    }

    @Override
    public void init() {
        super.init();
        if(drone == null) drone = UnitTypes.flare;
    }
    ...
    public void addStats(UnitType u, Table t) {
        ...
        t.table(title1 -> {
            title1.image(drone.uiIcon).size(iconXLarge).scaling(Scaling.fit);
            title1.add("[accent]" + drone.localizedName + (settings.getBool("console") ? "\n[gray]" + drone.name : "")).padLeft(5);
        });

        t.row();

        if(drone.description != null){
            var any = drone.stats.toMap().size > 0;

            if(any){
                t.add("@category.purpose").color(Pal.accent).fillX().padTop(10);
                t.row();
            }

            t.add("[lightgray]" + drone.displayDescription()).wrap().fillX().padLeft(any ? 10 : 0).width(500f).padTop(any ? 0 : 10).left();
            t.row();

            if(!drone.stats.useCategories && any){
                t.add("@category.general").fillX().color(Pal.accent);
                t.row();
            }
        }

        Stats stats = drone.stats;
    ...
    public void spawnDrone(Bullet b) {
        if(b.owner() instanceof Unitc c) {
            for (WeaponMount mount : c.mounts()) {
    ...
    public boolean droneAlive(DroneMount mount) {
        return mount.activeDrone != null && mount.activeDrone.isAdded() && !mount.activeDrone.dead();
    }
    ...
    public class DroneMount extends WeaponMount {

        public @Nullable Unit activeDrone = null;

        public DroneMount(Weapon weapon) {
            super(weapon);
        }
