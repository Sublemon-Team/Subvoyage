package subvoyage.type.block.turret;
...
public class LaserTurret extends PowerTurret implements LaserBlock {
    ...
    public void setBars() {
        super.setBars();
        addBar("laser", (entity) -> {
            if(entity instanceof LaserTurretBuild lb)
                return new Bar(
    ...
    public class LaserTurretBuild extends PowerTurretBuild implements LaserBuild {
        @Override
        public void updateEfficiencyMultiplier(){
            super.updateEfficiencyMultiplier();
            if(laserRequirement() > 0){
                float over = Math.max(laser() - laserRequirement(), 0.0F);
                efficiency *= Math.min(Mathf.clamp(laser() / laserRequirement()) + over / laserRequirement() * laserOverpowerScale, laserMaxEfficiency);
            }
        }
