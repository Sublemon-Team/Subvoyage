...
public class ContiniousLaserRangedBulletType extends ContinuousBulletType {
    public float fadeTime = 16f;
    public float lightStroke = 40f;
    public int divisions = 13;
    public Color[] colors = {SvPal.laserRed,SvPal.laserRed,Color.white, Color.white};
    public float strokeFrom = 2f, strokeTo = 0.5f, pointyScaling = 0.1f;
    public float backLength = 7f, frontLength = 35f;
    public float width = 9f, oscScl = 0.8f, oscMag = 1.5f;

    public ContiniousLaserRangedBulletType(float damage){
        this.damage = damage;
    }

    public ContiniousLaserRangedBulletType(){
    }
    ...
    public void draw(Bullet b){
        ...
        float realLength = Damage.findLength(b, length * fout * (float) b.data(), laserAbsorb, pierceCap);
    ...
    public float currentLength(Bullet b){
        float mult = (float) b.data();
        float fout = Mathf.clamp(b.time > b.lifetime - fadeTime ? 1f - (b.time - (lifetime - fadeTime)) / fadeTime : 1f);
        return length * mult * fout;
    }
}