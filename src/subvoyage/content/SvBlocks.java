...
public class SvBlocks{
    ...
    public static void loadCat() {
        subvoyage.content.block.SvEnvironment.load();
        SvStorage.load();
        SvProduction.load();
        SvCrafting.load();
        SvDistribution.load();
...
/*
    ...
    public static BuildVisibility atl(BuildVisibility v){
        return new BuildVisibility(() -> Vars.state == null || Vars.state.isMenu() || (v.visible() && Vars.state.rules.planet == SvPlanets.atlacian || Vars.state.rules.env == SvEnvironment.any || Vars.state.rules.planet == Planets.sun));
    }
